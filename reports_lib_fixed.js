...
.modal__footer { margin-top: 8px !important; flex-shrink: 0 !important; }
</style><style>.modal { padding: 12px !important; max-width: 95vw !important; width: 340px !important; } .modal__header { margin-bottom: 8px !important; } .modal__title { font-size: 1.1rem !important; }</style>
  ...
  `,T();const l=document.getElementById("ab-date"),c=document.getElementById("ab-time");l.addEventListener("change",()=>{const r=J(l.value,n);c.innerHTML='<option value="">Выберите время...</option>'+r.map(m=>`
    ...
    `,T(),document.querySelectorAll(".detail-btn").forEach(g=>{g.addEventListener("click",async()=>{const y=g.dataset.id,w=a.find(S=>String(S.id)===String(y));if(w){const S=w.washer_id||w.washerId?e.find(E=>String(E.id)===String(w.washer_id||w.washerId)):null,h=n.find(E=>String(E.id)===String(w.user_id||w.userId));await O({title:"Детали мойки 🚿",content:`<div style="display:flex; flex-direction:column; gap:4px; font-size:12px; line-height:1.1;">
    <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.04); padding:4px 8px; border-radius:8px;">
        <span style="font-size:1.2rem;">${"⭐".repeat(a)}</span>
        <b style="font-size:1.1rem; color:var(--color-primary);">${parseFloat(e).toLocaleString()} ₽</b>
    </div>
...
</div>`})}})})}async function W(t){const a=await d.getAll("services")||[];t.innerHTML=`
    ${await k()}
    <main class="page-content animate-fade-in">
      <div class="page-header">
